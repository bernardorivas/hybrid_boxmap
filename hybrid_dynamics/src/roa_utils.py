...
def compute_regions_of_attraction(
    ...
    if config.logging.verbose:
        logger.info(f"Computing regions of attraction for {len(morse_sets)} Morse sets...")
    ...
    for morse_idx, morse_set in enumerate(morse_sets):
        ...
        if config.logging.verbose:
            logger.info(f"  Morse set {morse_idx}: {len(morse_set)} boxes, ROA: {len(roa_boxes)} boxes")
...
def analyze_roa_coverage(
    ...
    overlap_percentage = (len(overlapping_boxes) / len(all_roa_boxes)) * 100 if all_roa_boxes else 0
    ...
    if config.logging.verbose:
        ...
        logger.info(f"  Covered by ROAs: {len(all_roa_boxes)} ({coverage_percentage:.1f}%)")
        logger.info(f"  Uncovered boxes: {len(uncovered_boxes)} ({100-coverage_percentage:.1f}%)")
        logger.info(f"  Overlapping boxes: {len(overlapping_boxes)} ({overlap_percentage:.1f}% of covered)")
