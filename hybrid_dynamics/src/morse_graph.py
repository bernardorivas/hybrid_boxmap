...
def visualize_morse_graph_comparison(G: nx.DiGraph, output_path: str = None) -> Tuple[nx.DiGraph, List[Set]]:
    """
    ...
    nx.draw(G, pos=pos1, with_labels=True, node_color='lightblue', 
            node_size=500, font_size=16, font_weight='bold', ax=ax1)
    nx.draw_networkx_edge_labels(G, pos1, edge_labels={(u,v): f'{u}→{v}' for u,v in G.edges()}, ax=ax1)
    ...
    if hasse.nodes():
        ...
        nx.draw(hasse, pos=pos2, labels=node_labels, with_labels=True, 
                node_color=node_colors, 
                node_size=1000, font_size=10, 
                font_weight='bold', ax=ax2)
        ax2.set_title("Morse Graph (Hasse Diagram)")
    else:
        ax2.text(0.5, 0.5, 'No non-trivial SCCs with ordering', 
...
def main_example():
    ...
    hasse, nontrivial_sccs = visualize_morse_graph_comparison(G, output_path=output_file)
