...
class DatasetCubifier:
    ...
    def __init__(self, 
        ...
        for i, (lower, upper) in enumerate(bounds):
            if lower >= upper:
                raise ValueError(f"Invalid bounds for dimension {i}: [{lower}, {upper}]")
    ...
    def _create_base_grid(self):
        ...
        logger.debug(f"Created {len(self.base_boxes)} base grid boxes for {self.dimension}D space")
    ...
    def cubify_trajectory(self, 
                         trajectory: HybridTrajectory, 
                         make_square: bool = True,
                         use_subdivision: bool = True) -> List[Union[Box, SquareBox]]:
        ...
        if all_states.shape[1] != self.dimension:
            raise ValueError(f"Trajectory dimension {all_states.shape[1]} != cubifier dimension {self.dimension}")
        
        return self.cubify_points(all_states, make_square=make_square, use_subdivision=use_subdivision)
    
    def cubify_points(self, 
        ...
        if points.shape[1] != self.dimension:
            raise ValueError(f"Points dimension {points.shape[1]} != cubifier dimension {self.dimension}")
        ...
        if use_subdivision and self.subdivision_levels > 0:
            ...
            if refined_boxes:
                ...
                logger.debug(f"Refined to {len(containing_boxes)} boxes after subdivision")
    ...
    def cubify_bounding_box(self, 
                           points: np.ndarray,
                           make_square: bool = True) -> Union[Box, SquareBox]:
    ...
    def plot_cubification(self, 
                         ...
                         box_style: dict = None,
                         trajectory_style: dict = None) -> plt.Axes:
        ...
        if isinstance(dataset, HybridTrajectory):
            ...
            for jump_time, (state_before, state_after) in zip(dataset.jump_times, dataset.jump_states):
    ...
    def get_coverage_statistics(self, 
        ...
        return {
            ...
            'efficiency': (covered_points / total_points) / (total_volume / domain_volume) if total_volume > 0 else 0
        }
    
    def __str__(self) -> str:
        """String representation of cubifier."""
        return (f"DatasetCubifier({self.dimension}D, resolution={self.base_resolution}, "
