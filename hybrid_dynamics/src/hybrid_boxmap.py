...
class HybridBoxMap(dict):
    ...
    def compute_cylindrical(
        ...
        """
        if grid.ndim != 4:
            raise ValueError("Cylindrical computation requires 4D grid [x, y, x_dot, y_dot]")
        ...
        if config.logging.verbose:
            ...
            logger.info(f"Sampling: {n_radial_samples} radial × {n_angular_samples} angular")
        ...
        x_dot_samples = np.linspace(x_dot_bounds[0], x_dot_bounds[1], n_velocity_samples)
        y_dot_samples = np.linspace(y_dot_bounds[0], y_dot_bounds[1], n_velocity_samples)
